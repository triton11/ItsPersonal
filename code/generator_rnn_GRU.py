...
n_characters = len(all_characters) # Plus EOS marker
#EOS = n_letters - 1
...
def char_tensor(string_in):
    ...
    # tensor = torch.zeros(len(string_in), n_characters).long()
    # for ci in range(len(string_in)):
    #     char = string_in[ci]
...
def random_training_set(category):    
    ...
    # if (len(line) > 1000):
    #     line = line[0:1000] 
    # line_input = char_tensor(line)
    # #print(line)
    # line_target = make_target(line)
    # #print(line_target)

    # return line_input, line_target
